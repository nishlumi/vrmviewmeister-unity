...
namespace UserHandleSpace
{
    public class CustomVrmaDataStore
    {
        ...
        public class LookAtData
        {
            ...
            public Vector3 position; // 各フレームの座標を保存
        ...
        public void CaptureLookAtDirection(ExportFrame frame)
        {
            ...
            // LookAtDataを作成（その瞬間のワールド座標 currentTargetPos を渡す）
        ...
        */
        ...
        private List<Vector3> GenerateLookAtTranslation(Transform root, List<CustomVrmaDataStore.ExportFrame> frames)
        {
            ...
            foreach (var frame in frames)
            {
                if (frame.lookAt != null)
                {
                    // VRMルート（足元）基準のローカル座標に変換（保存された各フレームの座標 position を使用）
        ...
        private int AddLookAtNode(CustomVrmaDataStore datastore)
        {
            ...
            Nodes.Add(datastore.m_lookAtNode); // Unity側のTransformもリストに加える
