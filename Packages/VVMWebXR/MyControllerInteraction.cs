...
{
    public class MyControllerInteraction : MonoBehaviour
    {
        private FixedJoint attachJoint = null;
        private Rigidbody currentRigidBody = null;
        private List<Rigidbody> contactRigidBodies = new List<Rigidbody>();

        [SerializeField] private List<string> targetTags = new List<string>() { "Interactable","IKHandle" };
      ...
      WebXRController.ButtonTypes.ButtonA
        ...
        private Vector3 previousLocalRot;
    ...
    private static Quaternion quat180 = Quaternion.Euler(0, 180, 0);
        ...
        public GameObject HandMenuOpener;
        public GameObject HandMenuButton1;
        public GameObject HandMenuButton2;

        public bool IsHandleMXR2 = false;
      ...
      if (inputProfileObject != null)
      {
        ...
        private void Update()
        {
            ...
            //---2024.01.20 add
            previousLocalRot = transform.localRotation.eulerAngles;
      ...
      else
      {
        inputProfileModelParent.SetActive(false);
            ...
            {
                int dir = 1;
                //---HandMenu when controller
                if (gameObject.GetComponent<WebXRController>().hand == WebXRControllerHand.RIGHT)
                {
                    dir = -1;
                }
                if (HandMenuButton1 != null) HandMenuButton1.transform.localPosition = new Vector3(0, 0.002f, -0.225f);
                HandMenuOpener.transform.localPosition = new Vector3(0, 0.002f, -0.17f);
                HandMenu.transform.localPosition = new Vector3(0.035f * dir, 0.05f, -0.211f);
                
                HandMenu.transform.localRotation = Quaternion.Euler(0, 0, 0);
            }
        }
      ...
      else
      {
        inputProfileHandModelParent.SetActive(false);
            ...
            else
            {
                ...
                if (HandMenuButton1 != null) HandMenuButton1.transform.localPosition = new Vector3(0, 0.002f, -0.175f);
                HandMenuOpener.transform.localPosition = new Vector3(0, 0.002f, -0.12f);
                HandMenu.transform.localPosition = new Vector3(0.035f * dir, -0.07f, -0.16f);

                HandMenu.transform.localRotation = Quaternion.Euler(0, 0, 90 * dir * -1);
            }
        }
      ...
      if (useInputProfile && loadedHandModel)
      {
        ...
        return;
          ...
          clone.SetLocalPositionAndRotation(rotationOffset * (handData.joints[i].position - handData.joints[0].position), rotationOffset * handData.joints[i].rotation);
                    ...
                    clone.name = System.Enum.GetName(typeof(WebXRHandJoint), (WebXRHandJoint)i);
    ...
    private void SetAxisValue(int index)
    {
      inputProfileModel.SetAxisValue(index, controller.GetAxisIndexValue(index));
        ...
        public void Pickup()
        {
            ...
            MouseOperationXR2 moxr2;
            if (currentRigidBody.transform.TryGetComponent<MouseOperationXR2>(out moxr2))
            {
                //currentRigidBody.transform.Translate((transform.position - previousPos) * Time.deltaTime, Space.World);
                Sequence seq = DOTween.Sequence();
                seq.Join(currentRigidBody.transform.DOMove((transform.position - previousPos), Time.deltaTime).SetRelative());
                //seq.Join(currentRigidBody.transform.DOLocalRotate(transform.localRotation.eulerAngles - previousLocalRot, Time.deltaTime).SetRelative());
                seq.Join(currentRigidBody.transform.DORotate(transform.rotation.eulerAngles - previousRot, Time.deltaTime).SetRelative());
